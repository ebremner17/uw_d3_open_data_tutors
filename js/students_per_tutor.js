$(document).ready(function () {
  ...
  d3.json("https://api.uwaterloo.ca/v2/terms/1159/enrollment.json?key=<insert_key_here>", function (error, students_full) {
    ...
    // Get the student data using alasql.
    students = alasql('SELECT students.subject as subject, students.catalog_number as catalog_number, SUM(tutors.tutors_count) as tutors_count, SUM(students.enrollment_total) as enrollment_total, (SUM(students.enrollment_total) / SUM(tutors.tutors_count) * 100) as ratio FROM ? as tutors INNER JOIN ? as students ON (tutors.subject = students.subject AND tutors.catalog_number = students.catalog_number) WHERE (tutors.tutors_count > 0) GROUP BY students.subject, students.catalog_number', [tutors_obj.data, students_full.data]);
    ...
    tip = d3.tip()
      ...
      .html(function (f) {
        ...
        return f.subject + f.catalog_number + "<br />Enrollment: " + f.enrollment_total + "<br />" + "Tutors: " + f.tutors_count;
      });
    ...
    svg.append("text")
      ...
      .text("Ratio of Tutors per Student");
